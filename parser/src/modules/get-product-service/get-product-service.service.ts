...
import { DataCard } from 'src/interfaces/search.interfaces';

@Injectable()
export class GetProductServiceService {

    async getProduct(article: string): Promise<{ data: DataCard, image: string }> {
        const dataBase = 11
        const data = [];
    ...
    async getProductImage(articleId): Promise<string> {
